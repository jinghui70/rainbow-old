...
public abstract class DaoUtils {
	...
	static {
		InputStream xsltStream = DaoUtils.class.getResourceAsStream("h2.xsl");
		try {
			...
			if (xsltStream != null)
				Closeables.closeQuietly(xsltStream);
		}
	}

	synchronized public static String transform(Model model) {
		try {
			String xmltext = Model.getXmlBinder().marshalString(model);
