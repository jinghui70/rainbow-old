...
public class DefaultCellRenderer implements KTableCellRenderer {
	...
	protected int m_alignment = SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_TOP;
	...
	protected void drawCellContent(GC gc, Rectangle rect, String text, Image img, Color textColor, Color backColor) {
		...
		SWTX.drawTextImage(gc, text, getAlignment(), img, getAlignment(), rect.x + 3, rect.y + 2, rect.width - 6,
			rect.height - 4);
	}
	 ...
	 * @param backColor The background color to use.
	 */
	protected void drawVerticalCellContent(GC gc, Rectangle rect, String text, Image img, Color textColor,
			Color backColor) {
		if (rect.height <= 0)
			rect.height = 1;
		if (rect.width <= 0)
	...
	private Image mirrorImage(Image source) {
		...
		ImageData resultData = new ImageData(sourceData.height, sourceData.width, sourceData.depth,
			sourceData.palette);
	...
	protected final void drawCommentSign(GC gc, Rectangle rect) {
		...
		gc.fillPolygon(new int[] { rect.x + rect.width - 4, rect.y + 1, rect.x + rect.width - 1, rect.y + 1,
